...
class ControllerExtensionPaymentPaysonCheckout2 extends Controller {
    ...
    public function index() {
        ...
        if (isset($this->request->post['payment_paysonCheckout2_secret_token'])) {
                $data['payment_paysonCheckout2_secret_token'] = $this->request->post['payment_paysonCheckout2_secret_token'];
        } elseif ($this->config->get('payment_paysonCheckout2_secret_token')) {
                $data['payment_paysonCheckout2_secret_token'] = $this->config->get('payment_paysonCheckout2_secret_token');
        } else {
                $data['payment_paysonCheckout2_secret_token'] = sha1(uniqid(mt_rand(), 1));
        }

        //$data['payment_paysonCheckout2_webhook_url'] = HTTPS_CATALOG . 'index.php?route=extension/payment/worldpay/webhook&token=' . $data['worldpay_secret_token'];

        $data['payment_paysonCheckout2_cron_job_url'] = HTTPS_CATALOG . 'index.php?route=extension/payment/paysonCheckout2/cron&token=' . $data['payment_paysonCheckout2_secret_token'];
    ...
    $this->load->model('setting/setting');
        ...
        try {
            ...
            foreach ($merchants as $merchant) {
                ...
                $checkoutClient = new \Payson\Payments\RecurringSubscriptionClient($connector); 
                // Make the call
    ...
    /*public function transaction() {
		...
		$data['transactions'][] = array(
					'transaction_id' => 'nada',
					'amount'         => 'todo',
					'payment_type'   => 'todoochnada',
					'payment_status' => 'hhhhhhh',
					'pending_reason' => 'dkdkdkdkdk',
					'date_added'     => date('now'));
           // }
        //}

        $this->response->setOutput($this->load->view('extension/payment/paysonCheckou_transaction', $data));
    }*/
}